...
const GroupChatDetailsModal = ({ open, onClose, chatId, onGroupDelete }) => {
  ...
  const getUsers = async () => {
    requestHandler(
      ...
      null, // On successful retrieval, set the users' state.
      res => {
        const { data } = res
        setUsers(data || [])
      }, // Use default alert for any error messages.
  ...
  const deleteGroupChat = async () => {
    ...
    requestHandler(
      ...
      null, // On successful deletion, trigger onGroupDelete and close any modals/dialogs.
      () => {
  ...
  const addParticipant = async () => {
    ...
    requestHandler(
      ...
      async () => await addParticipantToGroup(chatId, participantToBeAdded), // No loading callback provided, so passing `null`.
  ...
  const fetchGroupInformation = async () => {
    requestHandler(
      ...
      async () => await getGroupInfo(chatId), // Placeholder for a loading callback (currently set to null)
      null, // If the request is successful, destructure the response and set group details and the group name
      res => {
        const { data } = res
