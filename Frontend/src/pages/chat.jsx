import {
  ...
  import AddChatModal from "../components/messages/AddChatModal"
...
import { useSelector } from "react-redux"
  ...
  console.log("the unread message count",currentChat);
      ...
      requestHandler(
        ...
        async () => await getChatMessages(currentChat.current?._id || ""), // Set the state to loading while fetching the messages
        setLoadingMessages, // After fetching, set the chat messages to the state if available
        res => {
          const { data } = res
          setMessages(data || [])
    ...
    const sendChatMessage = async () => {
      ...
      await requestHandler(
        ...
        async () =>
          await sendMessage(
            // Chat ID or empty string if not available
            currentChat.current?._id || "", // Actual text message
            message, // Any attached files
            attachedFiles
          ),
        null, // On successful message sending, clear the message input and attached files, then update the UI
        res => {
